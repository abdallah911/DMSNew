...
namespace DMS_Authontication1.Controllers
{
    public class RayApprovalsController : Controller
    {
        ...
        public JsonResult Rays()
        {
            List<SERV_PROVIDERS_NEW> Rays = new List<SERV_PROVIDERS_NEW>();
            Rays = db.SERV_PROVIDERS_NEW.Where(x => x.PRV_TYPE == 4 && x.TERMINATE_FLAG != "Y").GroupBy(x => x.PR_CODE).Select(x => x.FirstOrDefault()).ToList();
            return new JsonResult { Data = Rays, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        }
        ...
        public JsonResult SavePrescription(PrescriptionViewModel data)
        {
            ...
            try
            {
                ...
                if (model != null)
                {
                    try
                    {
                        PostSMSData("New Rays has been added . If it is not used, please call 0226390390 ", model.Phone);
                    }
                    catch (Exception)
                    {
        ...
        public JsonResult DoctorDelete(long id, long roshitaid)
        {
            try
            {
                ...
                {
                    roshita.Manager = "Ray_Daily_Stop";
                }
        ...
        public JsonResult GetRayByCode(long userProvider, long code)
        {
