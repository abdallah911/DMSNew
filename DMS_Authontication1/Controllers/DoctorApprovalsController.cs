...
namespace DMS_Authontication1.Controllers
{
    public class DoctorApprovalsController : Controller
    {
        ...
        public JsonResult SavePrescription(PrescriptionViewModel data)
        {
            ...
            try
            {
                ...
                if (model != null)
                {
                    try
                    {
                        PostSMSData("New medicine has been added . If it is not used, please call 0226390390 ", model.Phone);
                    }
                    catch (Exception)
                    {
        ...
        public JsonResult SubmetChronic(List<Med_Medicine> data)
        {
            ...
            foreach (Med_Medicine item in data)
            {
                ...
                else
                {
                    if (CompareMedicine(item, newone))
                    {
                        ...
                        update.UNIT_PRICE = System.Math.Round(item.UNIT_PRICE.Value, 5);
                        ...
                        updateroshita.Amount = System.Math.Round(item.TOTAL_AMT.Value, 5);
            ...
            db.SaveChanges();
            var model = db.CardsSms.Where(c => c.CardId == Rosita.CardId).FirstOrDefault();
            if (model != null)
            {
                try
                {
